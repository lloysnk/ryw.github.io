...
function sparkle_destroy() {
  let t;
  for (; tiny.length; ) (t = tiny.pop()), t && document.body.removeChild(t);
  for (; star.length; ) (t = star.pop()), t && document.body.removeChild(t);
}
  ...
  window.addEventListener("resize", function () {
    ...
    (document.onmousemove = function (t) {
      if (sparkles_enabled && !t.buttons) {
        const e = Math.sqrt(
            Math.pow(t.movementX, 2) + Math.pow(t.movementY, 2)
          ),
...
function animate_sparkles(t = 60) {
  const e = 1e3 / t;
  let i = 0;
  for (let t = 0; t < star.length; t++) i += update_star(t);
  for (let t = 0; t < tiny.length; t++) i += update_tiny(t);
  0 !== i || sparkles_enabled || sparkle_destroy(),
    setTimeout("animate_sparkles(" + t + ")", e);
}
function create_star(t, e, i = 1) {
  if (t + 5 >= doc_width || e + 5 >= doc_height) return;
  ...
  for (let t = 0; t < sparkles; t++) {
    ...
    star_remaining_ticks[t] < n && ((n = star_remaining_ticks[t]), (s = t));
  }
  return (
    n && star_to_tiny(s),
    s >= 0
      ? ((star_remaining_ticks[s] = 2 * sparkle_lifetime),
        (star_x[s] = t),
        (star[s].style.left = t + "px"),
        (star_y[s] = e),
        (star[s].style.top = e + "px"),
        (star[s].style.clip = "rect(0px, 5px, 5px, 0px)"),
        (star[s].childNodes[0].style.backgroundColor = star[
          s
...
function update_star(t) {
  return (
    null !== star_remaining_ticks[t] &&
    ((star_remaining_ticks[t] -= 1),
    0 === star_remaining_ticks[t]
      ? (star_to_tiny(t), !1)
      : (star_remaining_ticks[t] === sparkle_lifetime &&
          (star[t].style.clip = "rect(1px, 4px, 4px, 1px)"),
        star_remaining_ticks[t] > 0 &&
        ((star_y[t] += 1 + 3 * Math.random()),
        (star_x[t] += ((t % 5) - 2) / 5),
        star_y[t] + 5 < doc_height && star_x[t] + 5 < doc_width)
          ? ((star[t].style.top = star_y[t] + "px"),
...
function star_to_tiny(t) {
  null !== star_remaining_ticks[t] &&
    (star_y[t] + 3 < doc_height &&
      star_x[t] + 3 < doc_width &&
      ((tiny_remaining_ticks[t] = 2 * sparkle_lifetime),
      (tiny_y[t] = star_y[t]),
      ...
      (tiny[t].style.backgroundColor =
        star[t].childNodes[0].style.backgroundColor),
      (star[t].style.visibility = "hidden"),
...
function update_tiny(t) {
  return (
    null !== tiny_remaining_ticks[t] &&
    ((tiny_remaining_ticks[t] -= 1),
    tiny_remaining_ticks[t] === sparkle_lifetime &&
      ((tiny[t].style.width = "1px"), (tiny[t].style.height = "1px")),
    tiny_remaining_ticks[t] > 0 &&
    ((tiny_y[t] += 1 + 2 * Math.random()),
    (tiny_x[t] += ((t % 4) - 2) / 4),
    tiny_y[t] + 3 < doc_height && tiny_x[t] + 3 < doc_width)
      ? ((tiny[t].style.top = tiny_y[t] + "px"),
